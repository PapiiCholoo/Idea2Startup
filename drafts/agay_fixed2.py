...
class UltraAccurateFloodPredictor:
    ...
    def __init__(self, config: HighAccuracyNagaCityConfig, n_particles: int = 2000):
        ...
        self.dx_m = config.dx * 111320  # meters per degree longitude
        self.dy_m = config.dy * 110540  # meters per degree latitude
    ...
    def ultra_high_accuracy_shallow_water_solver(self, particle_idx, precipitation_rate, dt):
        ...
        grad_Fh = np.gradient(F_h, self.dx_m, axis=1)
        grad_Fhu = np.gradient(F_hu, self.dx_m, axis=1)

        # ✅ Force all terms to the same shape as h
        target_shape = h.shape

        def match_shape(arr, target_shape):
            """Crop or pad array to match target_shape."""
            arr = np.array(arr)
            arr = arr[:target_shape[0], :target_shape[1]]
            if arr.shape != target_shape:
                pad_y = target_shape[0] - arr.shape[0]
                pad_x = target_shape[1] - arr.shape[1]
                arr = np.pad(arr, ((0, max(0, pad_y)), (0, max(0, pad_x))), mode="edge")
            return arr

        grad_Fh = match_shape(grad_Fh, target_shape)
        grad_Fhu = match_shape(grad_Fhu, target_shape)
        source_precip = match_shape(source_precip, target_shape)
        infiltration_loss = match_shape(infiltration_loss, target_shape)
        et_loss = match_shape(et_loss, target_shape)
        bed_slope_x = match_shape(bed_slope_x, target_shape)
        bed_slope_y = match_shape(bed_slope_y, target_shape)

        # Time derivatives with source terms
        dh_dt = -grad_Fh + source_precip - infiltration_loss - et_loss
        dhu_dt = -grad_Fhu - g * h * bed_slope_x + friction_u * h
        ...
        # Similar for v-direction (already simplified)
        dhv_dt = friction_v * h - g * h * bed_slope_y
    ...
    def _extract_state_vector(self, particle_idx):
        ...
        h_flat = self.water_depth[1:1+self.config.ny, 1:1+self.config.nx, particle_idx].flatten()
        return h_flat
...
def main_ultra_accurate():
    ...
    config = HighAccuracyNagaCityConfig(dx=0.00018, dy=0.00018)
   
    # Create ultra-accurate flood prediction system
    flood_predictor = UltraAccurateFloodPredictor(config, n_particles=5)  # High ensemble size
